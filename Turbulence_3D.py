...
def passot_pouquet_spectrum(k,lx):
    up = 0.5
    l11 = lx/3
    ke = np.sqrt(2*pi)/l11    
    #print('Ke = %.4f' % ke)
...
def turbulence_3d(xmin, xmax, ymin, ymax, zmin, zmax, nx, ny, nz):
    ...
    kmag = np.zeros([nx, ny, nz], dtype = complex)
    e_spec = np.zeros([nx, ny, nz], dtype = complex)
    ...
    for k in range(0,nz):
        ...
        for j in range(0,ny):
            ...
            for i in range(0,int(nx/2)):
                ...
                if (kmag[i,j,k] > kc):
                    continue
                e_spec[i,j,k], up,ke = passot_pouquet_spectrum(kmag,xmax)
...
#------------------------------------------------------------------------------
U = turbulence_3d( 0, 0.565, 0, 1.13, 0, 1.13, 16, 32, 32)
